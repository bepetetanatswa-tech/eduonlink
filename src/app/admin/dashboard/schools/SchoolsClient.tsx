...
const PLANS = ["free_school", "school_starter", "school_standard", "school_pro", "school_enterprise"];
...
export function SchoolsClient({ initialSchools }: { initialSchools: School[] }) {
  ...
  const [addForm, setAddForm] = useState({ name: "", province: PROVINCES[0], district: "", email: "", phone: "", subscription_plan: "free_school" });
  const [rejecting, setRejecting] = useState<School | null>(null);
  ...
  const decide = async (schoolId: string, decision: "approved" | "rejected", reason?: string) => {
    setDeciding(true);
    try {
      const res = await fetch("/api/admin/schools/decide", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ schoolId, decision, reason }),
      });
      const data = await res.json();
      if (!res.ok) { notify(data.error ?? "Could not save decision"); return; }
      setSchools((p) => p.map((s) => s.id === schoolId ? { ...s, status: decision, is_verified: decision === "approved", rejection_reason: decision === "rejected" ? (reason ?? null) : null } : s));
      notify(decision === "approved" ? "School approved ✓ Email sent." : "School rejected. Email sent.");
      setRejecting(null);
      setRejectReason("");
    } finally {
      setDeciding(false);
  ...
  const openEdit = (s: School) => { setEditing(s); setEditForm({ name: s.name, province: s.province ?? "", district: s.district ?? "", email: s.email ?? "", phone: s.phone ?? "", subscription_plan: s.subscription_plan }); };
        ...
        <div style={{ background: "rgba(77,127,255,0.05)", border: "1px solid rgba(77,127,255,0.2)", borderRadius: 16, padding: 20, display: "flex", flexDirection: "column", gap: 12 }}>
          ...
          <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: 10 }}>
            {[
              ...
              { label: "District", key: "district", placeholder: "e.g. Harare" },
