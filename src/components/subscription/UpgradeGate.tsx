...
const FEATURES: Record<string, { name: string; requiredPlan: string; emoji: string }> = {
  ...
  mock_exams:        { name: "Mock Exams",            requiredPlan: "Student Pro or Credits Pack", emoji: "🎯" },
  video_upload:      { name: "Video Upload",          requiredPlan: "Teacher Pro ($10/mo)",        emoji: "🎬" },
  ai_tools:          { name: "AI Lesson Builder",     requiredPlan: "Teacher Pro ($10/mo)",        emoji: "✨" },
  grade_book:        { name: "Grade Book",            requiredPlan: "Teacher Pro ($10/mo)",        emoji: "📒" },
  attendance:        { name: "Attendance System",     requiredPlan: "Teacher Pro ($10/mo)",        emoji: "✅" },
  earnings:          { name: "Earnings Dashboard",    requiredPlan: "Teacher Pro ($10/mo)",        emoji: "💰" },
  analytics:         { name: "Advanced Analytics",    requiredPlan: "School Standard ($60/mo)",    emoji: "📈" },
  custom_branding:   { name: "Custom Branding",       requiredPlan: "School Enterprise ($200/mo)", emoji: "🎨" },
};
...
export function UpgradeGate({ feature, hasAccess, children, subscriptionPath, compact = false }: Props) {
  ...
  const info = FEATURES[feature] ?? { name: feature, requiredPlan: "a paid plan", emoji: "🔒" };
  ...
  if (compact) {
    return (
      <a href={href} style={{ display: "inline-flex", alignItems: "center", gap: 6, padding: "7px 14px", borderRadius: 9, background: "rgba(77,127,255,0.08)", border: "1px solid rgba(77,127,255,0.2)", color: S.accent, fontSize: 12, fontWeight: 600, textDecoration: "none", cursor: "pointer" }}>
    ...
    <>
      <div style={{ position: "relative", userSelect: "none", cursor: "pointer" }} onClick={() => setShowModal(true)}>
        ...
        <div style={{ position: "absolute", inset: 0, display: "flex", flexDirection: "column", alignItems: "center", justifyContent: "center", background: "rgba(7,8,12,0.75)", borderRadius: 12, gap: 10 }}>
          <div style={{ width: 48, height: 48, borderRadius: "50%", background: "rgba(77,127,255,0.12)", border: "1px solid rgba(77,127,255,0.25)", display: "flex", alignItems: "center", justifyContent: "center" }}>
      ...
      {showModal && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.82)", zIndex: 200, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}
          ...
          <div style={{ background: "#0E1117", border: "1px solid rgba(77,127,255,0.2)", borderRadius: 20, padding: 36, maxWidth: 420, width: "100%", textAlign: "center" }}
            onClick={e => e.stopPropagation()}>
            <div style={{ fontSize: 56, marginBottom: 18 }}>{info.emoji}</div>
            <h3 style={{ fontSize: 20, fontWeight: 700, color: S.text, fontFamily: "'Space Grotesk',sans-serif", margin: "0 0 10px" }}>
              Unlock {info.name}
            </h3>
            ...
            <div style={{ display: "flex", gap: 10, justifyContent: "center" }}>
              <button onClick={() => setShowModal(false)}
                style={{ padding: "10px 20px", borderRadius: 10, background: "rgba(255,255,255,0.05)", border: `1px solid ${S.border}`, color: S.muted, fontSize: 13, cursor: "pointer" }}>
                Maybe later
              </button>
