...
export function ClassChat({ classId, profileId, userName, className, isTeacher = false, height = "calc(100vh - 80px)" }: Props) {
 ...
 const S = { bg: "#F2EEE3", card: "rgba(28,38,32,0.02)", border: "rgba(28,38,32,0.07)", accent: "#B1502B", text: "#1C2620", muted: "#566257", dim: "#6E7A6C" };
 ...
 <div style={{ width: 36, height: 36, borderRadius: 10, background: "linear-gradient(135deg,#B1502B22,#8F402222)", border: `1px solid ${S.accent}33`, display: "flex", alignItems: "center", justifyContent: "center" }}>
 ...
 <div>
 <p style={{ fontSize: 14, fontWeight: 600, color: S.text, fontFamily: "inherit" }}>{className}</p>
 {!isOnline ? (
 <p style={{ fontSize: 11, color: "#A3311E" }}>Offline — messages will send when reconnected</p>
 ) : connStatus === "connected" ? (
 <p style={{ fontSize: 11, color: S.dim }}>{onlineCount} online</p>
 ) : (
 <p style={{ fontSize: 11, color: "#A9873F" }}>{connStatus === "connecting" ? "Connecting…" : "Reconnecting…"}</p>
 )}
 </div>
 </div>
 ...
 <button onClick={toggleNotifMute} title={notifMuted ? "Unmute notifications" : "Mute notifications"} style={{ width: 32, height: 32, borderRadius: 8, border: `1px solid ${S.border}`, background: notifMuted ? "rgba(169,135,63,0.15)" : "transparent", color: notifMuted ? "#A9873F" : S.muted, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 14 }}>
 <IconBell size={15} />
 </button>
 </div>
 </div>
 ...
 style={{ width: "100%", background: "rgba(28,38,32,0.04)", border: `1px solid ${S.border}`, borderRadius: 8, padding: "7px 12px", fontSize: 13, color: S.text, outline: "none" }}
 />
 </div>
 )}
 ...
 <span style={{ fontSize: 11, color: S.dim, background: "rgba(28,38,32,0.05)", padding: "3px 12px", borderRadius: 20 }}>{dayLabel(msg.created_at)}</span>
 </div>
 )}
 ...
 {/* Reply quote */}
 {msg.parent && (
 <div style={{ background: isOwn ? "rgba(28,38,32,0.08)" : "rgba(28,38,32,0.05)", borderLeft: `2px solid ${S.accent}`, borderRadius: "6px 6px 0 0", padding: "5px 10px", fontSize: 11, color: S.muted, maxWidth: "100%" }}>
 ...
 : <div style={{ width: 220, height: 120, borderRadius: 8, background: "rgba(28,38,32,0.04)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 11, color: S.dim, marginBottom: 4 }}>Loading image…</div>
 )}
 ...
 {/* Hover action row */}
 {showEmoji === msg.id && (
 <div style={{ position: "absolute", [isOwn ? "left" : "right"]: "-120px", top: 0, background: "#F2EEE3", border: `1px solid ${S.border}`, borderRadius: 10, padding: "4px 6px", display: "flex", gap: 4, zIndex: 20, boxShadow: "0 8px 24px rgba(0,0,0,0.4)", flexWrap: "wrap", width: 116 }}>
 {EMOJIS.map(e => (
 ...
 onMouseEnter={(ev) => (ev.currentTarget.style.background = "rgba(28,38,32,0.08)")}
 ...
 <button onClick={() => toggleStar(msg.id)} style={{ background: "none", border: "none", cursor: "pointer", padding: "3px", color: starredIds.has(msg.id) ? "#A9873F" : S.muted, fontSize: 11 }} title={starredIds.has(msg.id) ? "Unstar" : "Star"}>{starredIds.has(msg.id) ? "★" : "☆"}</button>
 {isOwn && <button onClick={() => deleteMessage(msg.id)} style={{ background: "none", border: "none", cursor: "pointer", padding: "3px", color: "#A3311E", fontSize: 11 }} title="Delete">✕</button>}
 {isTeacher && !isOwn && (
 <>
 ...
 <button onClick={() => setShowMute(msg.sender_id)} style={{ background: "none", border: "none", cursor: "pointer", color: "#A9873F", fontSize: 11, padding: "3px" }} title="Mute"></button>
 </>
 )}
 </div>
 )}
 ...
 style={{ background: r.users.includes(profileId) ? `${S.accent}25` : "rgba(28,38,32,0.06)", border: `1px solid ${r.users.includes(profileId) ? S.accent + "50" : S.border}`, borderRadius: 12, padding: "2px 7px", fontSize: 12, cursor: "pointer", color: S.muted, display: "flex", gap: 3, alignItems: "center" }}>
 ...
 <div style={{ background: "rgba(163,49,30,0.08)", border: "1px solid rgba(163,49,30,0.3)", borderRadius: "12px 4px 12px 12px", padding: "8px 12px" }}>
 <p style={{ fontSize: 13, color: S.text, lineHeight: 1.5, margin: 0, wordBreak: "break-word" }}>{p.content}</p>
 </div>
 <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
 <span style={{ fontSize: 10, color: "#A3311E" }}>
 ...
 <div style={{ background: "#F2EEE3", border: `1px solid ${S.border}`, borderRadius: 14, padding: 24, width: 280 }}>
 <p style={{ fontSize: 14, color: S.text, marginBottom: 16 }}>Mute this student from class chat?</p>
 <div style={{ display: "flex", gap: 8 }}>
 <button onClick={() => muteUser(showMute)} style={{ flex: 1, padding: "8px", borderRadius: 8, background: "#A9873F", border: "none", color: "#fff", cursor: "pointer", fontSize: 13, fontWeight: 600 }}>Mute</button>
 <button onClick={() => setShowMute(null)} style={{ flex: 1, padding: "8px", borderRadius: 8, background: "rgba(28,38,32,0.06)", border: `1px solid ${S.border}`, color: S.text, cursor: "pointer", fontSize: 13 }}>Cancel</button>
 </div>
 </div>
 </div>
 )}
 ...
 {showInfo && (
 <div style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50 }}>
 <div style={{ background: "#F2EEE3", border: `1px solid ${S.border}`, borderRadius: 14, padding: 20, width: 340, maxHeight: 480, display: "flex", flexDirection: "column" }}>
 ...
 <div key={m.id} style={{ width: "100%", aspectRatio: "1", borderRadius: 8, background: "rgba(28,38,32,0.04)" }} />
 )
 ))}
 </div>
 );
 ...
 {files.map((m) => (
 <a key={m.id} href={signedUrls[m.file_url!] ?? "#"} target="_blank" rel="noreferrer" style={{ display: "flex", gap: 8, alignItems: "center", color: signedUrls[m.file_url!] ? S.accent : S.dim, textDecoration: "none", fontSize: 12, padding: "6px 8px", borderRadius: 8, background: "rgba(28,38,32,0.03)" }}>
 ...
 <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
 {starred.map((m) => (
 <div key={m.id} style={{ padding: "8px 10px", borderRadius: 8, background: "rgba(28,38,32,0.03)", border: `1px solid ${S.border}` }}>
 <p style={{ fontSize: 11, color: "#A9873F", margin: "0 0 3px", fontWeight: 600 }}>{m.sender?.full_name}</p>
 ...
 {forwarding && (
 <div style={{ position: "absolute", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50 }}>
 <div style={{ background: "#F2EEE3", border: `1px solid ${S.border}`, borderRadius: 14, padding: 20, width: 300, maxHeight: 400, display: "flex", flexDirection: "column" }}>
 <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 12 }}>
 ...
 myClasses.map((c) => (
 <button key={c.id} onClick={() => forwardTo(c.id)} style={{ textAlign: "left", padding: "10px 12px", borderRadius: 10, border: `1px solid ${S.border}`, background: "rgba(28,38,32,0.02)", color: S.text, fontSize: 13, cursor: "pointer" }}>
 {c.name}
 </button>
 ))
 )}
 ...
 {isMuted && (
 <p style={{ fontSize: 12, color: "#A9873F", textAlign: "center", marginBottom: 8 }}>You have been muted by the teacher.</p>
 )}

 {/* Reply quote */}
 {replyTo && (
 <div style={{ background: "rgba(28,38,32,0.04)", borderLeft: `2px solid ${S.accent}`, borderRadius: 6, padding: "5px 10px", marginBottom: 8, display: "flex", justifyContent: "space-between", alignItems: "center" }}>
 <div>
 ...
 style={{ width: 36, height: 36, borderRadius: 10, border: `1px solid ${S.border}`, background: "rgba(28,38,32,0.04)", color: uploading ? S.accent : S.muted, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}
 >
 ...
 style={{ width: 36, height: 36, borderRadius: 10, border: `1px solid ${recording ? "#A3311E" : S.border}`, background: recording ? "rgba(163,49,30,0.15)" : "rgba(28,38,32,0.04)", color: recording ? "#A3311E" : S.muted, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, position: "relative" }}
 >
 ...
 {recording && <span style={{ position: "absolute", bottom: -16, fontSize: 9, color: "#A3311E", whiteSpace: "nowrap" }}>{recordSeconds}s</span>}
 </button>
 <input
 value={input}
 ...
 style={{ flex: 1, background: "rgba(28,38,32,0.04)", border: `1px solid ${S.border}`, borderRadius: 10, padding: "9px 14px", fontSize: 13, color: S.text, outline: "none", resize: "none" }}
 />
 <button
 onClick={sendMessage}
 disabled={!input.trim() || sending || isMuted}
 style={{ width: 36, height: 36, borderRadius: 10, background: (!input.trim() || sending || isMuted) ? "rgba(177,80,43,0.2)" : S.accent, border: "none", color: "#fff", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "background 0.15s" }}
 >
