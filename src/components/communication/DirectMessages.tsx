...
function Avatar({ name, url, size = 32 }: { name: string; url?: string | null; size?: number }) {
 ...
 return (
 <div style={{ width: size, height: size, borderRadius: "50%", background: "linear-gradient(135deg,#B1502B,#8F4022)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: size * 0.36, fontWeight: 700, color: "#fff", flexShrink: 0 }}>
 {initials}
 </div>
 );
}
...
export function DirectMessages({ profileId, userRole, allowedRoles, heightOffset = 0 }: Props) {
 ...
 const S = { bg: "#F2EEE3", card: "#0A0B10", border: "rgba(28,38,32,0.07)", accent: "#B1502B", text: "#1C2620", muted: "#566257", dim: "#6E7A6C" };
 ...
 <span style={{ position: "absolute", bottom: -1, right: -1, width: 10, height: 10, borderRadius: "50%", background: otherOnline ? "#1F4738" : "#6E7A6C", border: `2px solid ${S.card}` }} />
 </div>
 <div style={{ flex: 1 }}>
 <p style={{ fontSize: 13, fontWeight: 600, color: S.text, margin: 0 }}>{selected.full_name}</p>
 <p style={{ fontSize: 11, color: !isOnline ? "#A3311E" : connStatus !== "connected" ? "#A9873F" : otherTyping ? S.accent : S.dim, textTransform: otherTyping ? "none" : "capitalize", margin: 0 }}>
 ...
 <button onClick={() => setShowStarred(!showStarred)} title="Starred messages" style={{ width: 32, height: 32, borderRadius: 8, border: `1px solid ${S.border}`, background: showStarred ? "rgba(169,135,63,0.15)" : "transparent", color: showStarred ? "#A9873F" : S.muted, cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, fontSize: 14 }}>
 ★
 </button>
 <button onClick={toggleMute} title={muted ? "Unmute notifications" : "Mute notifications"} style={{ background: "none", border: `1px solid ${S.border}`, borderRadius: 8, color: muted ? "#A9873F" : S.muted, cursor: "pointer", fontSize: 13, padding: "6px 8px", display: "flex", alignItems: "center" }}>
 <IconBell size={14} />
 </button>
 ...
 {/* Input */}
 {sendError && (
 <p style={{ margin: "0 14px", fontSize: 12, color: "#A3311E" }}>{sendError}</p>
 )}
 {isBlocked && (
 <p style={{ margin: "0 14px 8px", fontSize: 12, color: "#A9873F", textAlign: "center" }}>
 ...
 <p style={{ margin: "0 14px 8px", fontSize: 12, color: "#A3311E", textAlign: "center" }}>
 This request was declined. You can&apos;t message {selected.full_name.split("")[0]} right now.
 </p>
 )}
 {dmRequestStatus === "pending_received" && (
 <div style={{ margin: "0 14px 10px", padding: "10px 14px", background: "rgba(177,80,43,0.06)", border: `1px solid ${S.accent}30`, borderRadius: 10, display: "flex", alignItems: "center", justifyContent: "space-between", gap: 10 }}>
 <span style={{ fontSize: 12, color: S.text }}>{selected.full_name} wants to message you</span>
 <div style={{ display: "flex", gap: 6, flexShrink: 0 }}>
 <button onClick={() => respondToRequest(true)} style={{ padding: "5px 12px", borderRadius: 7, background: "#1F4738", border: "none", color: "#F2EEE3", fontSize: 11, fontWeight: 600, cursor: "pointer" }}>Accept</button>
 <button onClick={() => respondToRequest(false)} style={{ padding: "5px 12px", borderRadius: 7, background: "rgba(163,49,30,0.1)", border: "1px solid rgba(163,49,30,0.3)", color: "#A3311E", fontSize: 11, fontWeight: 600, cursor: "pointer" }}>Decline</button>
 </div>
 </div>
 )}
 ...
 style={{ width: 36, height: 36, borderRadius: 10, border: `1px solid ${S.border}`, background: "rgba(28,38,32,0.04)", color: uploading ? S.accent : S.muted, cursor: (isBlocked || dmRequestStatus !== "accepted") ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}
 >
 ...
 style={{ width: 36, height: 36, borderRadius: 10, border: `1px solid ${recording ? "#A3311E" : S.border}`, background: recording ? "rgba(163,49,30,0.15)" : "rgba(28,38,32,0.04)", color: recording ? "#A3311E" : S.muted, cursor: (isBlocked || dmRequestStatus !== "accepted") ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, position: "relative" }}
 >
 ...
 style={{ flex: 1, background: "rgba(28,38,32,0.04)", border: `1px solid ${S.border}`, borderRadius: 10, padding: "9px 14px", fontSize: 13, color: S.text, outline: "none" }}
 />
 <button onClick={sendMessage} disabled={!input.trim() || sending || isBlocked || (dmRequestStatus !== "accepted" && dmRequestStatus !== "none")}
 style={{ width: 36, height: 36, borderRadius: 10, background: (!input.trim() || isBlocked) ? "rgba(177,80,43,0.2)" : S.accent, border: "none", color: "#fff", cursor: isBlocked ? "not-allowed" : "pointer", display: "flex", alignItems: "center", justifyContent: "center" }}>
 ...
 <div style={{ background: "#F2EEE3", border: `1px solid ${S.border}`, borderRadius: 16, padding: 20, width: 320, maxHeight: 480 }}>
 ...
 onMouseEnter={(e) => (e.currentTarget.style.background = "rgba(28,38,32,0.04)")}
 ...
 <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50 }}>
 <div style={{ background: "#F2EEE3", border: `1px solid ${S.border}`, borderRadius: 14, padding: 24, width: 300 }}>
 ...
 <div style={{ display: "flex", gap: 8 }}>
 <button onClick={toggleBlock} style={{ flex: 1, padding: "8px", borderRadius: 8, background: blockedByMe ? "#1F4738" : "#A3311E", border: "none", color: "#F2EEE3", cursor: "pointer", fontSize: 13, fontWeight: 600 }}>
 {blockedByMe ? "Unblock" : "Block"}
 </button>
 <button onClick={() => setShowBlockConfirm(false)} style={{ flex: 1, padding: "8px", borderRadius: 8, background: "rgba(28,38,32,0.06)", border: `1px solid ${S.border}`, color: S.text, cursor: "pointer", fontSize: 13 }}>Cancel</button>
 </div>
 </div>
 </div>
 );
 ...
 <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", display: "flex", alignItems: "center", justifyContent: "center", zIndex: 50 }}>
 <div style={{ background: "#F2EEE3", border: `1px solid ${S.border}`, borderRadius: 14, padding: 20, width: 340, maxHeight: 440, display: "flex", flexDirection: "column" }}>
 ...
 <p style={{ fontSize: 11, color: "#A9873F", margin: "0 0 3px", fontWeight: 600 }}>{m.sender_id === profileId ? "You" : m.sender.full_name}</p>
