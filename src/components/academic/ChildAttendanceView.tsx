...
export function ChildAttendanceView({ childId, childName }: { childId: string; childName: string }) {
  ...
  const rateColor = rate === null ? S.dim : rate >= 80 ? "#1F4738" : rate >= 60 ? "#A9873F" : "#A3311E";

  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      <h3 style={{ fontSize: 15, fontWeight: 600, color: S.text, fontFamily: "inherit", margin: 0 }}>{childName}</h3>

      {loading ? (
        <p style={{ fontSize: 13, color: S.dim }}>Loading attendance…</p>
      ) : records.length === 0 ? (
        <div style={{ background: "rgba(28,38,32,0.02)", border: `1px solid ${S.border}`, borderRadius: 12, padding: "24px", textAlign: "center" }}>
          <p style={{ color: S.dim, fontSize: 13 }}>No attendance recorded yet.</p>
        </div>
      ) : (
        <>
          <div style={{ display: "flex", alignItems: "center", gap: 12, flexWrap: "wrap" }}>
            <div style={{ background: "rgba(28,38,32,0.02)", border: `1px solid ${S.border}`, borderRadius: 12, padding: "14px 16px", flex: "0 0 auto" }}>
              <p style={{ fontSize: 11, color: S.dim, margin: "0 0 4px" }}>Attendance rate (last 90 records)</p>
              <p style={{ fontSize: 24, fontWeight: 700, color: rateColor, fontFamily: "inherit", margin: 0 }}>{rate}%</p>
            </div>
            {atRisk && (
              <div style={{ background: "rgba(163,49,30,0.1)", border: "1px solid rgba(163,49,30,0.25)", borderRadius: 10, padding: "10px 14px" }}>
                <p style={{ fontSize: 12, fontWeight: 600, color: "#A3311E", margin: 0 }}>⚠ At-risk: below 80% attendance</p>
              </div>
            )}
          </div>
          ...
          <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
            {records.slice(0, 20).map((r, i) => (
              <div key={`${r.date}-${i}`} style={{ background: "rgba(28,38,32,0.02)", border: `1px solid ${S.border}`, borderRadius: 12, padding: "10px 16px", display: "flex", alignItems: "center", justifyContent: "space-between", gap: 12, flexWrap: "wrap" }}>
                <div>
