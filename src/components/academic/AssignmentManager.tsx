...
export function AssignmentManager({ profileId, lockedClassId }: { profileId: string; lockedClassId?: string }) {
 ...
 style={{ padding: "9px 16px", borderRadius: 9, background: "rgba(28,38,32,0.05)", border: `1px solid ${S.border}`, color: S.muted, fontSize: 13, cursor: "pointer" }}>
 ← Back
 </button>
 )}
 </div>
 ...
 {/* Create form */}
 {view === "create" && (
 <div style={{ background: "rgba(28,38,32,0.02)", border: `1px solid ${S.border}`, borderRadius: 16, padding: 24, display: "flex", flexDirection: "column", gap: 16 }}>
 <h3 style={{ fontSize: 15, fontWeight: 700, color: S.text, fontFamily: "inherit", margin: 0 }}>New Assignment</h3>
 ...
 style={{ padding: "7px 14px", borderRadius: 8, background: "rgba(28,38,32,0.04)", border: `1px solid ${S.border}`, color: S.muted, fontSize: 12, cursor: "pointer", alignSelf: "flex-start" }}>
 + Add Criterion
 </button>
 </div>
 </div>
 ...
 <div style={{ width: 36, height: 20, borderRadius: 10, background: fAllowLate ? S.accent : "rgba(28,38,32,0.1)", position: "relative", transition: "background 0.2s", flexShrink: 0 }}>
 ...
 <button onClick={() => setView("list")} style={{ padding: "9px 18px", borderRadius: 9, background: "rgba(28,38,32,0.05)", border: `1px solid ${S.border}`, color: S.muted, fontSize: 13, cursor: "pointer" }}>Cancel</button>
 ...
 return (
 <div key={a.id} style={{ background: "rgba(28,38,32,0.02)", border: `1px solid ${S.border}`, borderRadius: 12, padding: "16px 20px" }}>
 <div style={{ display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 12 }}>
 <div style={{ flex: 1 }}>
 <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 4 }}>
 <p style={{ fontSize: 14, fontWeight: 600, color: S.text, margin: 0 }}>{a.title}</p>
 {overdue && <span style={{ fontSize: 10, padding: "2px 7px", borderRadius: 20, background: "rgba(163,49,30,0.15)", color: "#A3311E", fontWeight: 600 }}>Past Due</span>}
 </div>
 ...
 {a.due_date && <span style={{ fontSize: 11, color: overdue ? "#A3311E" : S.dim }}>Due: {new Date(a.due_date).toLocaleDateString("en-GB", { day: "numeric", month: "short", year: "numeric", hour: "2-digit", minute: "2-digit" })}</span>}
 <span style={{ fontSize: 11, color: a.allow_late ? "#1F4738" : "#A3311E" }}>{a.allow_late ? "Late OK" : "No late"}</span>
 </div>
 </div>
 <div style={{ display: "flex", gap: 8, flexShrink: 0 }}>
 <button onClick={() => loadSubmissions(a)}
 style={{ padding: "8px 14px", borderRadius: 9, background: "rgba(177,80,43,0.1)", border: `1px solid rgba(177,80,43,0.2)`, color: "#B1502B", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
 Grade
 </button>
 <button onClick={() => deleteAssignment(a.id)}
 style={{ padding: "8px 14px", borderRadius: 9, background: "rgba(163,49,30,0.08)", border: `1px solid rgba(163,49,30,0.2)`, color: "#A3311E", fontSize: 12, cursor: "pointer" }}>
 Delete
 </button>
 </div>
 </div>
 ...
 <h3 style={{ fontSize: 15, fontWeight: 700, color: S.text, fontFamily: "inherit", margin: "0 0 4px" }}>{selectedAssignment.title}</h3>
 ...
 return (
 <div style={{ marginTop: 10, padding: "10px 14px", background: "rgba(177,80,43,0.05)", border: `1px solid rgba(177,80,43,0.1)`, borderRadius: 9 }}>
 <p style={{ fontSize: 11, fontWeight: 600, color: "#B1502B", margin: "0 0 4px" }}>Marking Rubric</p>
 {criteria ? (
 ...
 return (
 <div key={s.id} style={{ background: "rgba(28,38,32,0.02)", border: `1px solid ${S.border}`, borderRadius: 12, padding: "16px 20px" }}>
 <div style={{ display: "flex", alignItems: "center", gap: 10, marginBottom: 12 }}>
 <div style={{ width: 32, height: 32, borderRadius: 9, background: "linear-gradient(135deg, #B1502B, #8F4022)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 13, fontWeight: 700, color: "#fff", flexShrink: 0 }}>
 ...
 {s.is_late && <span style={{ fontSize: 10, padding: "1px 6px", borderRadius: 20, background: "rgba(255,165,0,0.15)", color: "#A9873F", fontWeight: 600 }}>LATE</span>}
 <span style={{ fontSize: 10, padding: "1px 6px", borderRadius: 20,
 background: s.status === "graded" ? "rgba(31,71,56,0.15)" : "rgba(28,38,32,0.06)",
 color: s.status === "graded" ? "#1F4738" : S.dim, fontWeight: 600 }}>
 ...
 {s.content && (
 <div style={{ background: "rgba(28,38,32,0.03)", border: `1px solid ${S.border}`, borderRadius: 9, padding: "10px 14px", marginBottom: 12 }}>
 ...
 style={{ display: "inline-flex", alignItems: "center", gap: 6, fontSize: 12, color: S.accent, background: "rgba(177,80,43,0.08)", border: `1px solid rgba(177,80,43,0.15)`, padding: "6px 12px", borderRadius: 8, textDecoration: "none", marginBottom: 12 }}>
 View submitted file
 </a>
 )}
 ...
 <p style={{ fontSize: 12, color: "#A9873F", margin: "0 0 12px" }}>
 ...
 style={{ marginLeft: 8, padding: "2px 10px", borderRadius: 6, background: "rgba(169,135,63,0.1)", border: "1px solid rgba(169,135,63,0.25)", color: "#A9873F", fontSize: 11, cursor: "pointer" }}>
 Apply penalty
 </button>
 </p>
 )}
