...
import { IconFlask, IconPencilCheck } from "@/components/icons";
...
const STAGE_DEFS = [
 ...
 { num: 5, name: "Product Creation", icon: "", desc: "Create your actual product or presentation based on your plan and research." },
...
function renderAIText(text: string) {
 ...
 ? <strong key={j} style={{ color: "#1C2620" }}>{p.slice(2, -2)}</strong>
 : <span key={j}>{p}</span>
 )}
 {i < text.split("\n").length - 1 && <br />}
 </span>
...
export function HBCWorkflow({ project, stages: initialStages, profileId }: Props) {
 ...
 <div style={{ background: "rgba(28,38,32,0.02)", border: "1px solid rgba(28,38,32,0.06)", borderRadius: 14, padding: "14px 16px", marginBottom: 4 }}>
 <p style={{ fontSize: 11, color: "#6E7A6C", marginBottom: 4, textTransform: "uppercase", letterSpacing: "0.08em", fontWeight: 600 }}>Progress</p>
 <p style={{ fontSize: 22, fontWeight: 700, color: "#1C2620", fontFamily: "inherit", margin: "0 0 8px" }}>{completedCount}/6</p>
 <div style={{ width: "100%", height: 6, background: "rgba(28,38,32,0.06)", borderRadius: 3, overflow: "hidden" }}>
 <div style={{ height: "100%", width: `${(completedCount / 6) * 100}%`, background: "linear-gradient(90deg, #B1502B, #1F4738)", borderRadius: 3, transition: "width 0.4s ease" }} />
 </div>
 <p style={{ fontSize: 10, color: "#6E7A6C", marginTop: 6 }}>stages submitted</p>
 </div>
 ...
 background: isActive ? "rgba(177,80,43,0.12)" : "rgba(28,38,32,0.02)",
 border: `1px solid ${isActive ? "rgba(177,80,43,0.3)" : "rgba(28,38,32,0.05)"}`,
 ...
 background: isApproved ? "rgba(31,71,56,0.12)" : isDone ? "rgba(177,80,43,0.12)" : "rgba(28,38,32,0.04)",
 border: `1px solid ${isApproved ? "rgba(31,71,56,0.3)" : isDone ? "rgba(177,80,43,0.3)" : "rgba(28,38,32,0.08)"}`,
 }}>
 {isApproved ? "✓" : isDone ? "●" : def.icon}
 </div>
 <div style={{ flex: 1, minWidth: 0 }}>
 <p style={{ fontSize: 10, fontWeight: 700, color: "#6E7A6C", margin: 0, textTransform: "uppercase", letterSpacing: "0.05em" }}>Stage {def.num}</p>
 <p style={{ fontSize: 11, fontWeight: 600, color: isActive ? "#1C2620" : "#566257", margin: 0, whiteSpace: "nowrap", overflow: "hidden", textOverflow: "ellipsis" }}>{def.name}</p>
 {isApproved && <p style={{ fontSize: 9, color: "#1F4738", margin: 0 }}>Teacher Approved</p>}
 {isDone && !isApproved && <p style={{ fontSize: 9, color: "#B1502B", margin: 0 }}>Submitted</p>}
 </div>
 </button>
 );
 })}
 ...
 {/* Stage Header */}
 <div style={{ background: "rgba(28,38,32,0.02)", border: "1px solid rgba(28,38,32,0.06)", borderRadius: 16, padding: "16px 20px" }}>
 <div style={{ display: "flex", alignItems: "center", gap: 12, marginBottom: 8 }}>
 <div style={{ width: 40, height: 40, borderRadius: "11px", background: "rgba(177,80,43,0.1)", border: "1px solid rgba(177,80,43,0.2)", display: "flex", alignItems: "center", justifyContent: "center", fontSize: 20 }}>
 {currentStageDef.icon}
 </div>
 <div>
 <p style={{ fontSize: 10, color: "#6E7A6C", margin: 0, textTransform: "uppercase", letterSpacing: "0.08em" }}>Stage {currentStageDef.num} of 6</p>
 <h3 style={{ fontSize: 16, fontWeight: 700, color: "#1C2620", fontFamily: "inherit", margin: 0 }}>{currentStageDef.name}</h3>
 </div>
 {currentStage.is_approved && (
 <div style={{ marginLeft: "auto", padding: "4px 12px", borderRadius: 20, background: "rgba(31,71,56,0.1)", border: "1px solid rgba(31,71,56,0.25)", fontSize: 11, fontWeight: 600, color: "#1F4738" }}>
 ✓ Teacher Approved
 </div>
 )}
 </div>
 <p style={{ fontSize: 13, color: "#566257", lineHeight: 1.6, margin: 0 }}>{currentStageDef.desc}</p>
 {currentStage.teacher_comment && (
 <div style={{ marginTop: 12, padding: "10px 14px", background: "rgba(31,71,56,0.06)", border: "1px solid rgba(31,71,56,0.15)", borderRadius: 10 }}>
 <p style={{ fontSize: 11, fontWeight: 600, color: "#1F4738", marginBottom: 4 }}> Teacher&apos;s Comment</p>
 <p style={{ fontSize: 13, color: "#1C2620", margin: 0, lineHeight: 1.6 }}>{currentStage.teacher_comment}</p>
 </div>
 )}
 </div>
 ...
 background: aiLoading === "blueprint" ? "rgba(169,135,63,0.15)" : "rgba(169,135,63,0.1)",
 border: "1px solid rgba(169,135,63,0.3)", color: "#A9873F",
 ...
 <><span></span> Generate Stage Blueprint</>
 )}
 </button>
 <button
 onClick={getFeedback}
 ...
 background: aiLoading === "feedback" ? "rgba(177,80,43,0.15)" : "rgba(177,80,43,0.1)",
 border: "1px solid rgba(177,80,43,0.3)", color: "#B1502B",
 fontSize: 13, fontWeight: 600, transition: "all 0.15s",
 display: "flex", alignItems: "center", justifyContent: "center", gap: 8,
 ...
 {currentStage.ai_feedback && (
 <div style={{
 background: isBlueprint ? "rgba(169,135,63,0.06)" : "rgba(177,80,43,0.06)",
 border: `1px solid ${isBlueprint ? "rgba(169,135,63,0.2)" : "rgba(177,80,43,0.2)"}`,
 borderRadius: 14, padding: "16px 18px",
 }}>
 <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 12 }}>
 <span style={{ display: "inline-flex", color: isBlueprint ? "#A9873F" : "#B1502B" }}>{isBlueprint ? <IconFlask size={18} /> : <IconPencilCheck size={18} />}</span>
 <p style={{ fontSize: 12, fontWeight: 700, color: isBlueprint ? "#A9873F" : "#B1502B", margin: 0, textTransform: "uppercase", letterSpacing: "0.06em" }}>
 {isBlueprint ? "Sir Taks — Stage Blueprint (Your Planning Guide)" : "Sir Taks — Feedback on Your Work"}
 </p>
 </div>
 {isBlueprint && (
 <div style={{ marginBottom: 10, padding: "8px 12px", background: "rgba(169,135,63,0.08)", borderRadius: 8, display: "flex", gap: 8, alignItems: "flex-start" }}>
 <span></span>
 <p style={{ fontSize: 11, color: "#A9873F", margin: 0, lineHeight: 1.5 }}>
 ...
 <div style={{ fontSize: 13, color: "#1C2620", lineHeight: 1.75 }}>
 ...
 {/* Content Editor */}
 <div style={{ background: "rgba(28,38,32,0.02)", border: "1px solid rgba(28,38,32,0.06)", borderRadius: 14, overflow: "hidden" }}>
 <div style={{ padding: "12px 16px", borderBottom: "1px solid rgba(28,38,32,0.05)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
 <p style={{ fontSize: 12, fontWeight: 600, color: "#566257", margin: 0 }}>Your Work — Stage {currentStageDef.num}</p>
 {currentStage.submitted_at && (
 <span style={{ fontSize: 10, color: "#B1502B", padding: "2px 8px", background: "rgba(177,80,43,0.1)", borderRadius: 6, border: "1px solid rgba(177,80,43,0.2)" }}>
 ...
 fontSize: 14, color: "#1C2620", lineHeight: 1.7, fontFamily: "inherit",
 ...
 color: notification.type === "success" ? "#1F4738" : "#A3311E",
 fontSize: 13,
 }}>
 ...
 background: "rgba(28,38,32,0.04)", border: "1px solid rgba(28,38,32,0.1)",
 color: "#566257", transition: "all 0.15s",
 }}
 >
 ...
 background: currentStage.submitted_at ? "rgba(31,71,56,0.08)" : "rgba(31,71,56,0.15)",
 border: "1px solid rgba(31,71,56,0.3)", color: "#1F4738",
 ...
 <div style={{ display: "flex", gap: 10, justifyContent: "space-between", borderTop: "1px solid rgba(28,38,32,0.06)", paddingTop: 16 }}>
 <button
 ...
 background: "rgba(28,38,32,0.03)", border: "1px solid rgba(28,38,32,0.08)",
 color: activeStage === 0 ? "#3A3D4E" : "#566257",
 }}
 >
 ← Previous Stage
 </button>
 ...
 background: "rgba(177,80,43,0.1)", border: "1px solid rgba(177,80,43,0.25)",
 color: activeStage === STAGE_DEFS.length - 1 ? "#3A3D4E" : "#B1502B",
 }}
 >
 Next Stage →
 </button>
