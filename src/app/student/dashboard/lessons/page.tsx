...
export default async function StudentLessonsPage() {
 ...
 {(courses ?? []).length === 0 ? (
 <div style={{ background: "rgba(28,38,32,0.02)", border: "1px solid rgba(28,38,32,0.06)", borderRadius: 16, padding: "52px 24px", textAlign: "center" }}>
 <div style={{ fontSize: 44, marginBottom: 14 }}></div>
 <p style={{ fontSize: 15, fontWeight: 600, color: "#566257", marginBottom: 8 }}>No courses published yet</p>
 <p style={{ fontSize: 12, color: "#6E7A6C" }}>Your teacher or school admin will publish courses here.</p>
 </div>
 ) : (
 ...
 return (
 <div key={c.id} style={{ background: "rgba(28,38,32,0.02)", border: "1px solid rgba(28,38,32,0.06)", borderRadius: 18, overflow: "hidden", display: "flex", flexDirection: "column" }}>
 {/* Header */}
 <div style={{ padding: "18px 18px 14px", background: `${accentColor}08`, borderBottom: "1px solid rgba(28,38,32,0.05)" }}>
 ...
 <p style={{ fontSize: 14, fontWeight: 700, color: "#1C2620", fontFamily: "inherit", margin: 0, lineHeight: 1.3 }}>{c.title}</p>
 {c.price > 0 && (
 <span style={{ fontSize: 10, fontWeight: 700, color: "#A9873F", background: "rgba(169,135,63,0.1)", border: "1px solid rgba(169,135,63,0.25)", padding: "1px 7px", borderRadius: 20 }}>${c.price.toFixed(2)}</span>
 )}
 </div>
 <p style={{ fontSize: 11, color: "#566257", margin: "3px 0 0" }}>{c.subject} · {c.grade_level}</p>
 </div>
 </div>
 {c.description && <p style={{ fontSize: 12, color: "#6E7A6C", marginTop: 10, lineHeight: 1.5 }}>{c.description}</p>}
 </div>

 {/* Progress */}
 {mats.length > 0 && (
 <div style={{ padding: "10px 18px", borderBottom: "1px solid rgba(28,38,32,0.04)" }}>
 <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 5 }}>
 <span style={{ fontSize: 10, color: "#6E7A6C" }}>Progress</span>
 <span style={{ fontSize: 10, fontWeight: 600, color: pct === 100 ? "#1F4738" : "#1C2620" }}>{done}/{mats.length} · {pct}%</span>
 </div>
 <div style={{ height: 4, background: "rgba(28,38,32,0.06)", borderRadius: 2 }}>
 <div style={{ height: "100%", width: `${pct}%`, background: pct === 100 ? "#1F4738" : accentColor, borderRadius: 2, transition: "width 0.4s" }} />
 </div>
 </div>
 )}
 ...
 {mats.length === 0 ? (
 <p style={{ fontSize: 12, color: "#6E7A6C", textAlign: "center", padding: "12px 0" }}>No materials added yet</p>
 ) : (
 <CourseAccessGate
 courseId={c.id}
 courseTitle={c.title}
 ...
 <div style={{ background: "rgba(177,80,43,0.05)", border: "1px solid rgba(177,80,43,0.1)", borderRadius: 12, padding: "12px 16px", display: "flex", gap: 10, alignItems: "center" }}>
 <span style={{ fontSize: 16 }}></span>
 <p style={{ fontSize: 12, color: "#6E7A6C", margin: 0, lineHeight: 1.5 }}>
 Need help with any of these topics? <Link href="/student/dashboard/ai-tutor" style={{ color: "#B1502B", textDecoration: "none", fontWeight: 600 }}>Ask Sir Taks AI</Link> for step-by-step explanations.
 </p>
 </div>
 </div>
 );
