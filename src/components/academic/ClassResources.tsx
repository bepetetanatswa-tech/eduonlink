...
export function ClassResources({ classId, profileId, isTeacher }: { classId: string; profileId: string; isTeacher: boolean }) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 12 }}>
      {isTeacher && (
        <div style={{ background: "rgba(255,255,255,0.02)", border: `1px solid ${S.border}`, borderRadius: 12, padding: 16, display: "flex", flexDirection: "column", gap: 10 }}>
          <input
            value={title}
            onChange={(e) => setTitle(e.target.value)}
            placeholder="Title (optional — defaults to file name)"
            style={{ padding: "9px 12px", background: "rgba(255,255,255,0.04)", border: `1px solid ${S.border}`, borderRadius: 9, color: S.text, fontSize: 13, outline: "none" }}
          />
          <input
            ref={fileInputRef}
            type="file"
      ...
      {resources.length === 0 ? (
        ...
        resources.map((r) => (
          <div key={r.id} style={{ background: "rgba(255,255,255,0.02)", border: `1px solid ${S.border}`, borderRadius: 12, padding: "12px 16px", display: "flex", alignItems: "center", gap: 12 }}>
