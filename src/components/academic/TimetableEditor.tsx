...
interface SlotForm { classId: string; subject: string; teacherId: string; startTime: string; endTime: string; room: string }
...
export function TimetableEditor({ schoolId }: { schoolId: string; profileId?: string }) {
  ...
  const [form, setForm] = useState<SlotForm>({ classId: "", subject: "", teacherId: "", startTime: "08:00", endTime: "08:45", room: "" });
  ...
  const openSlot = (day: number, period: number) => {
    setForm({ classId: classes[0]?.id ?? "", subject: "", teacherId: "", startTime: "08:00", endTime: "08:45", room: "" });
    setEditing({ day, period });
  };

  const saveSlot = async () => {
    if (!form.subject.trim() || !editing) return;
    setSaving(true);
    const cls = classes.find(c => c.id === form.classId);
    await (supabase.from("timetable_slots") as any).upsert({
      school_id: schoolId,
      class_id: form.classId || null,
      subject_name: form.subject.trim() || cls?.subject || "Lesson",
  ...
  const deleteSlot = async (day: number, period: number) => {
    await (supabase.from("timetable_slots") as any)
      .delete().eq("school_id", schoolId).eq("day_of_week", day).eq("period_number", period);
    setRefreshKey(k => k + 1);
  };
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      <div style={{ display: "flex", gap: 10, alignItems: "center" }}>
        <p style={{ fontSize: 13, color: S.dim, margin: 0 }}>Click any slot to assign a lesson. Changes apply instantly.</p>
      </div>
            ...
            <div key={p} style={{ display: "grid", gridTemplateColumns: `80px repeat(5, 1fr)`, gap: 6, marginBottom: 6 }}>
              ...
              {[0, 1, 2, 3, 4].map(d => (
                <div key={d} onClick={() => openSlot(d, p)}
                  style={{ minHeight: 60, borderRadius: 10, border: `1px dashed ${S.border}`, background: "rgba(255,255,255,0.01)", cursor: "pointer", display: "flex", alignItems: "center", justifyContent: "center", transition: "all 0.15s" }}
                  onMouseEnter={e => { (e.currentTarget as HTMLElement).style.background = "rgba(77,127,255,0.06)"; (e.currentTarget as HTMLElement).style.borderColor = "rgba(77,127,255,0.3)"; }}
                  onMouseLeave={e => { (e.currentTarget as HTMLElement).style.background = "rgba(255,255,255,0.01)"; (e.currentTarget as HTMLElement).style.borderColor = S.border; }}>
      ...
      <div>
        <h3 style={{ fontSize: 14, fontWeight: 600, color: S.text, fontFamily: "'Space Grotesk',sans-serif", margin: "0 0 12px" }}>Current Timetable</h3>
      ...
      {editing && (
        <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
          <div style={{ background: "#0E1117", border: `1px solid ${S.border}`, borderRadius: 18, padding: 28, width: "100%", maxWidth: 440, display: "flex", flexDirection: "column", gap: 14 }}>
            ...
            <div>
              <label style={{ fontSize: 11, fontWeight: 600, color: S.muted, display: "block", marginBottom: 5 }}>Class</label>
              <select value={form.classId} onChange={e => setForm(f => ({ ...f, classId: e.target.value }))} style={{ ...inp, cursor: "pointer" }}>
                <option value="" style={{ background: "#0E1117" }}>— No specific class —</option>
                {classes.map(c => <option key={c.id} value={c.id} style={{ background: "#0E1117" }}>{c.name}</option>)}
              </select>
            </div>
            <div>
