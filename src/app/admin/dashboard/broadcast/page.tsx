...
export default function BroadcastPage() {
 ...
 const inputStyle: React.CSSProperties = { width: "100%", padding: "10px 12px", background: "rgba(28,38,32,0.04)", border: `1px solid ${S.border}`, borderRadius: 10, color: S.text, fontSize: 13, outline: "none", boxSizing: "border-box" };

 return (
 <div style={{ maxWidth: 720, display: "flex", flexDirection: "column", gap: 24 }}>
 <div>
 <h2 style={{ fontSize: 20, fontWeight: 700, color: S.text, fontFamily: "inherit", margin: 0 }}>Broadcast</h2>
 <p style={{ fontSize: 12, color: S.dim, marginTop: 4 }}>Send announcements and notifications to all users or a specific role</p>
 </div>

 {sent && (
 <div style={{ padding: "12px 16px", borderRadius: 12, background: "rgba(31,71,56,0.1)", border: "1px solid rgba(31,71,56,0.25)", color: "#1F4738", fontSize: 13, fontWeight: 500 }}>
 ✓ Broadcast sent to {sent.count} users
 </div>
 )}
 {sendErr && (
 <div style={{ padding: "12px 16px", borderRadius: 12, background: "rgba(163,49,30,0.1)", border: "1px solid rgba(163,49,30,0.25)", color: "#A3311E", fontSize: 13, fontWeight: 500 }}>
 ⚠ {sendErr}
 </div>
 )}

 <form onSubmit={handleSend} style={{ background: "rgba(28,38,32,0.02)", border: `1px solid ${S.border}`, borderRadius: 16, padding: "24px", display: "flex", flexDirection: "column", gap: 16 }}>
 {/* Emergency toggle */}
 <div onClick={() => setIsEmergency(!isEmergency)} style={{ display: "flex", alignItems: "center", gap: 12, padding: "12px 16px", background: isEmergency ? "rgba(163,49,30,0.1)" : "rgba(28,38,32,0.02)", border: `1px solid ${isEmergency ? "rgba(163,49,30,0.35)" : S.border}`, borderRadius: 12, cursor: "pointer", transition: "all 0.15s" }}>
 <div style={{ width: 40, height: 22, borderRadius: 11, background: isEmergency ? "#A3311E" : "rgba(28,38,32,0.1)", position: "relative", transition: "background 0.2s", flexShrink: 0 }}>
 ...
 <div>
 <p style={{ fontSize: 13, fontWeight: 600, color: isEmergency ? "#A3311E" : S.muted, margin: 0 }}>Emergency Broadcast</p>
 ...
 style={{ padding: "12px", borderRadius: 12, background: isEmergency ? "#A3311E" : S.accent, border: "none", color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer", opacity: (sending || !title.trim() || !message.trim()) ? 0.5 : 1, fontFamily: "inherit" }}>
 ...
 {/* Recent broadcasts */}
 <div>
 <h3 style={{ fontSize: 15, fontWeight: 600, color: S.text, fontFamily: "inherit", margin: "0 0 12px" }}>Recent Broadcasts</h3>
 <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
 {recent.length === 0 && <p style={{ fontSize: 13, color: S.dim }}>No broadcasts yet.</p>}
 {recent.map(a => (
 <div key={a.id} style={{ background: "rgba(28,38,32,0.02)", border: `1px solid ${a.is_emergency ? "rgba(163,49,30,0.25)" : S.border}`, borderRadius: 12, padding: "14px 16px" }}>
 <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 4 }}>
 <div style={{ display: "flex", alignItems: "center", gap: 8 }}>
 {a.is_emergency && <span style={{ fontSize: 10, padding: "2px 8px", borderRadius: 20, background: "rgba(163,49,30,0.15)", color: "#A3311E", fontWeight: 700 }}>EMERGENCY</span>}
