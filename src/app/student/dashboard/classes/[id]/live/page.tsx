...
export default async function StudentLivePage({ params }: Props) {
  ...
  if (profile.role === "student") {
    ...
    if (plan.limits.liveClasses === false) {
      return (
        <div style={{ maxWidth: 480, margin: "60px auto", textAlign: "center", background: "rgba(255,255,255,0.02)", border: "1px solid rgba(255,255,255,0.06)", borderRadius: 16, padding: 40 }}>
          <div style={{ fontSize: 36, marginBottom: 10 }}>🔒</div>
          <h2 style={{ fontSize: 18, fontWeight: 700, color: "#CDD6F4", fontFamily: "'Space Grotesk',sans-serif", margin: "0 0 8px" }}>Live classes are a Student Pro feature</h2>
          <p style={{ fontSize: 13, color: "#8892B0", margin: "0 0 20px" }}>Upgrade to join live sessions with your teacher in real time.</p>
          <Link href="/student/dashboard/subscription" style={{ display: "inline-block", padding: "10px 24px", borderRadius: 10, background: "#4D7FFF", color: "#fff", fontSize: 13, fontWeight: 600, textDecoration: "none" }}>
            Upgrade Now
          </Link>
        </div>
      );
