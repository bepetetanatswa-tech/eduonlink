...
import { ECOCASH_NUMBER, ECOCASH_NAME } from "@/lib/subscription/plans";
...
export function CoursePurchase({ courseId, courseTitle, price, onClose }: Props) {
      ...
      <div style={{ background: "#0E1117", border: `1px solid ${S.border}`, borderRadius: 18, padding: 26, width: "100%", maxWidth: 460, display: "flex", flexDirection: "column", gap: 14, maxHeight: "90vh", overflowY: "auto" }}>
        {done ? (
          <div style={{ textAlign: "center", padding: "12px 0" }}>
            ...
            <p style={{ fontSize: 13, color: S.muted, margin: "0 0 16px" }}>Your ${price} payment for &quot;{courseTitle}&quot; is under review. You&apos;ll get access as soon as it&apos;s approved.</p>
          ...
          <>
            <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center" }}>
              <h3 style={{ fontSize: 16, fontWeight: 700, color: S.text, margin: 0 }}>Buy &quot;{courseTitle}&quot;</h3>
            ...
            <div style={{ background: "rgba(245,166,35,0.06)", border: "1px solid rgba(245,166,35,0.2)", borderRadius: 12, padding: 14 }}>
              <p style={{ fontSize: 12, color: S.muted, margin: "0 0 6px" }}>Send <strong style={{ color: S.accent }}>${price}</strong> to EcoCash <strong style={{ color: S.accent, fontFamily: "monospace" }}>{ECOCASH_NUMBER}</strong> ({ECOCASH_NAME}), then fill in the details below.</p>
            </div>
            <div>
            ...
            <button onClick={handleSubmit} disabled={submitting}
              style={{ padding: "12px", borderRadius: 10, background: submitting ? "rgba(245,166,35,0.4)" : S.accent, border: "none", color: "#fff", fontSize: 14, fontWeight: 700, cursor: submitting ? "not-allowed" : "pointer" }}>
