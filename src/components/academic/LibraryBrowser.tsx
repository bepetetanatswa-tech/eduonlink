...
export function LibraryBrowser({ profileId, canUpload }: { profileId: string; canUpload: boolean }) {
 ...
 <option value="" style={{ background: "#F2EEE3" }}>All levels</option>
 {LEVELS.map(l => <option key={l.v} value={l.v} style={{ background: "#F2EEE3" }}>{l.l}</option>)}
 </select>
 <select value={type} onChange={(e) => setType(e.target.value)} style={inp}>
 <option value="" style={{ background: "#F2EEE3" }}>All types</option>
 {TYPES.map(t => <option key={t.v} value={t.v} style={{ background: "#F2EEE3" }}>{t.l}</option>)}
 </select>
 <button onClick={() => setShowBookmarksOnly((p) => !p)}
 style={{ padding: "9px 14px", borderRadius: 9, background: showBookmarksOnly ? "rgba(169,135,63,0.15)" : "rgba(28,38,32,0.04)", border: `1px solid ${showBookmarksOnly ? "rgba(169,135,63,0.4)" : S.border}`, color: showBookmarksOnly ? "#A9873F" : S.muted, fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
 ★ Bookmarked
 </button>
 {canUpload && (
 ...
 return (
 <div key={r.id} style={{ background: "rgba(28,38,32,0.02)", border: `1px solid ${S.border}`, borderRadius: 14, padding: 16, display: "flex", flexDirection: "column", gap: 8 }}>
 <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", gap: 8 }}>
 <div style={{ display: "flex", gap: 6, flexWrap: "wrap" }}>
 <span style={{ fontSize: 10, color: S.accent, background: "rgba(177,80,43,0.1)", padding: "2px 8px", borderRadius: 20 }}>{TYPES.find(t => t.v === r.resource_type)?.l}</span>
 <span style={{ fontSize: 10, color: S.muted, background: "rgba(28,38,32,0.05)", padding: "2px 8px", borderRadius: 20 }}>{LEVELS.find(l => l.v === r.level)?.l}</span>
 {isNew && <span style={{ fontSize: 10, color: "#1F4738", background: "rgba(31,71,56,0.1)", padding: "2px 8px", borderRadius: 20 }}>New</span>}
 {isTop && <span style={{ fontSize: 10, color: "#A9873F", background: "rgba(169,135,63,0.1)", padding: "2px 8px", borderRadius: 20 }}> Popular</span>}
 </div>
 <button onClick={() => toggleBookmark(r.id)} style={{ background: "none", border: "none", cursor: "pointer", fontSize: 16, color: bookmarks.has(r.id) ? "#A9873F" : S.dim, flexShrink: 0 }}>
 {bookmarks.has(r.id) ? "★" : "☆"}
 </button>
 </div>
