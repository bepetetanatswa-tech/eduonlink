...
export function PushPermissionPrompt() {
 ...
 background: "#F2EEE3", border: "1px solid rgba(177,80,43,0.25)", borderRadius: 16,
 padding: 18, boxShadow: "0 20px 60px rgba(0,0,0,0.5)",
 }}>
 <p style={{ fontSize: 13, fontWeight: 700, color: "#1C2620", margin: "0 0 6px" }}> Stay in the loop</p>
 <p style={{ fontSize: 12, color: "#566257", margin: "0 0 14px", lineHeight: 1.5 }}>
 Allow EduOnLink to send you notifications for messages, assignments, and live classes?
 </p>
 {error && <p style={{ fontSize: 11, color: "#A3311E", margin: "0 0 10px" }}>{error}</p>}
 <div style={{ display: "flex", gap: 8 }}>
 <button onClick={dismiss} style={{ flex: 1, padding: "8px", borderRadius: 8, background: "rgba(28,38,32,0.05)", border: "1px solid rgba(28,38,32,0.1)", color: "#566257", fontSize: 12, cursor: "pointer" }}>
 Not now
 </button>
 <button onClick={allow} disabled={busy} style={{ flex: 1, padding: "8px", borderRadius: 8, background: "#B1502B", border: "none", color: "#fff", fontSize: 12, fontWeight: 600, cursor: busy ? "not-allowed" : "pointer", opacity: busy ? 0.7 : 1 }}>
 {busy ? "…" : "Allow"}
 </button>
 </div>
 </div>
