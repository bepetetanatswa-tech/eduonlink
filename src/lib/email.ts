...
const FROM = process.env.EMAIL_FROM || "Educonnect <onboarding@resend.dev>";
...
<body style="margin:0;padding:0;background:#07080C;font-family:-apple-system,'Segoe UI',sans-serif;">
  <div style="max-width:520px;margin:0 auto;padding:32px 24px;">
    <p style="color:#4D7FFF;font-weight:700;font-size:18px;letter-spacing:-0.02em;margin:0 0 24px;">Educonnect</p>
    ...
    <p style="color:#4A5170;font-size:11px;margin-top:20px;">Educonnect — Zimbabwe's Future Learns Here</p>
  </div>
</body>
</html>`;
...
export function schoolSubmittedEmail(schoolName: string) {
  return wrapper(
    ...
    `<p>Thanks for registering <strong style="color:#CDD6F4;">${schoolName}</strong> on Educonnect. Your application is now pending review by our team.</p><p>We'll email you as soon as a decision is made — this usually takes 1-2 business days.</p>`
  );
}
...
export function teacherInviteEmail(schoolName: string, inviteUrl: string) {
  return wrapper(
    ...
    `<p>${schoolName} has invited you to join Educonnect as a teacher.</p><p><a href="${inviteUrl}" style="color:#4D7FFF;">Accept invite & register →</a></p>`
  );
}
...
export function teacherApprovedEmail(fullName: string) {
  return wrapper(
    "You're verified! 🎉",
    `<p>Hi <strong style="color:#CDD6F4;">${fullName}</strong>, your teacher account has been verified. You now have full access to teaching features on Educonnect.</p>`
  );
}
