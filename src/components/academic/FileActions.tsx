    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 4 }}>
      <div style={{ display: "flex", gap: 6 }}>
        ...
        <button onClick={handleDownload} disabled={downloading}
          style={{ ...btnBase, background: "rgba(0,229,163,0.08)", border: "1px solid rgba(0,229,163,0.2)", color: "#00E5A3", opacity: downloading ? 0.6 : 1 }}>
          ⬇ {downloading ? "…" : "Download"}
        </button>
      </div>
      {blocked && (
        <div style={{ display: "flex", alignItems: "center", gap: 8, fontSize: 11, color: "#F5A623" }}>
          <span>{blocked}</span>
          <button onClick={() => router.push("/student/dashboard/subscription")}
            style={{ background: "none", border: "none", color: "#F5A623", textDecoration: "underline", cursor: "pointer", fontSize: 11, padding: 0 }}>
            Upgrade
          </button>
        </div>
      )}
