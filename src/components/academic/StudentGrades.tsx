    ...
    <div style={{ display: "flex", flexDirection: "column", gap: 20 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill,minmax(140px,1fr))", gap: 10 }}>
        {[
          ...
          { label: "Average", value: avg() ? `${avg()}%` : "—", color: "#F5A623" },
          { label: "A Grades", value: filtered.filter(g => g.grade === "A").length, color: "#00E5A3" },
        ].map(stat => (
      ...
      {[1, 2, 3].filter(t => term === "all" || t === term).map(t => {
        ...
        return (
          <div key={t}>
            <h3 style={{ fontSize: 14, fontWeight: 700, color: S.text, fontFamily: "'Space Grotesk',sans-serif", margin: "0 0 12px" }}>Term {t}</h3>
            <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
              {termGrades.map(g => {
                const gl = g.grade ? { l: g.grade, c: GRADE_COLOR[g.grade] ?? S.muted } : null;
                const pct = g.score !== null ? g.score : null;
                return (
                  <div key={g.id} style={{ background: "rgba(255,255,255,0.02)", border: `1px solid ${S.border}`, borderRadius: 12, padding: "14px 16px" }}>
                    <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
                      ...
                      {pct !== null && (
                        <div style={{ textAlign: "right" }}>
                          <p style={{ fontSize: 18, fontWeight: 700, color: gl?.c ?? S.muted, fontFamily: "'Space Grotesk',sans-serif", margin: 0 }}>{pct}%</p>
                        </div>
                      )}
                    </div>
                    {pct !== null && (
