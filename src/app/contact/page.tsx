    ...
    <div className="min-h-screen bg-edu-paper margin-rule">
      ...
      <section className="pt-40 pb-12 md:pt-48">
        <div className="container-edu max-w-2xl text-center">
          <h1
            className="font-display font-semibold text-edu-ink mb-5"
            style={{ fontSize: "clamp(32px, 6vw, 52px)", lineHeight: 1.08 }}
          >
            We&apos;d love to hear from you.
          </h1>
          <p className="text-base leading-relaxed max-w-xl mx-auto text-edu-slate-600">
      ...
      <section className="pb-16">
        <div className="container-edu max-w-2xl grid grid-cols-1 sm:grid-cols-2 gap-3">
          <a href={`mailto:${EMAIL}`} className="flex items-center gap-4 p-5 rounded border border-edu-slate-200 hover:border-edu-slate-400 transition-colors duration-150">
            ...
            <div>
              <p className="text-xs uppercase tracking-wider mb-1 text-edu-slate-500">Email</p>
              <p className="text-sm font-semibold text-edu-ink">{EMAIL}</p>
            </div>
          </a>
          ...
          <div className="flex items-center gap-4 p-5 rounded border border-edu-slate-200">
            ...
            <div>
              <p className="text-xs uppercase tracking-wider mb-1 text-edu-slate-500">Phone</p>
              <div className="flex flex-col gap-0.5">
                {PHONE_NUMBERS.map((num) => (
                  <a key={num} href={`tel:+263${num.replace(/\s/g, "").slice(1)}`} className="text-sm font-semibold text-edu-ink">
                    {num}
                  </a>
                ))}
              </div>
      ...
      <section className="pb-28">
        <div className="container-edu max-w-lg">
          <div className="border border-edu-slate-200 rounded p-6 md:p-8 flex flex-col gap-4">
            {sent ? (
              <div className="text-center py-6">
                <div className="w-12 h-12 rounded-full flex items-center justify-center mx-auto mb-3 bg-edu-bottle-100 border border-edu-bottle-200">
                  <IconCheck size={22} className="text-edu-bottle" strokeWidth={2.2} />
                </div>
              ...
              <form onSubmit={handleSubmit} className="flex flex-col gap-4">
                {error && (
                  <div className="p-3 rounded text-sm bg-edu-clay-100 border border-edu-clay-200 text-edu-clay-dark">
                    {error}
                  </div>
                )}
                ...
                <div className="flex flex-col gap-1.5">
                  <label className="text-sm font-medium text-edu-slate-600">Message</label>
                  <textarea
                    rows={5}
                    value={message}
