...
const S = {
 border: "rgba(28,38,32,0.07)", text: "#1C2620", muted: "#566257",
 dim: "#6E7A6C", accent: "#B1502B",
};
const inp: React.CSSProperties = {
 width: "100%", padding: "9px 12px", background: "rgba(28,38,32,0.04)",
...
export function LessonCreator({ profileId }: { profileId: string }) {
 ...
 const typeIcon: Record<string, React.ComponentType<{ size?: number }>> = { text: IconFileText, video: IconRadio, pdf: IconFileText, embed: IconRadio };
 ...
 <button onClick={openNewCourse} style={{ padding: "10px", borderRadius: 10, background: S.accent, border: "none", color: "#fff", fontSize: 13, fontWeight: 600, cursor: "pointer", fontFamily: "inherit" }}>
 + New Course
 </button>
 ...
 background: selected?.id === c.id ? "rgba(177,80,43,0.12)" : "rgba(28,38,32,0.02)",
 border: `1px solid ${selected?.id === c.id ? "rgba(177,80,43,0.3)" : S.border}`,
 ...
 background: c.is_published ? "rgba(31,71,56,0.15)" : "rgba(28,38,32,0.06)",
 color: c.is_published ? "#1F4738" : S.dim, flexShrink: 0 }}>
 {c.is_published ? "Live" : "Draft"}
 </span>
 </div>
 <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between", marginTop: 6 }}>
 <p style={{ fontSize: 11, color: S.dim, margin: 0 }}>{c.materials.length} material{c.materials.length !== 1 ? "s" : ""}</p>
 <p style={{ fontSize: 11, fontWeight: 700, color: c.price > 0 ? "#A9873F" : S.dim, margin: 0 }}>{c.price > 0 ? `$${c.price.toFixed(2)}` : "Free"}</p>
 </div>
 </div>
 ))}
 </div>
 ...
 <>
 {/* Course header */}
 <div style={{ background: "rgba(28,38,32,0.02)", border: `1px solid ${S.border}`, borderRadius: 14, padding: "16px 20px" }}>
 <div style={{ display: "flex", alignItems: "center", justifyContent: "space-between" }}>
 <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
 <span style={{ fontSize: 28 }}>{selected.thumbnail_emoji}</span>
 <div>
 <h3 style={{ fontSize: 16, fontWeight: 700, color: S.text, fontFamily: "inherit", margin: 0 }}>{selected.title}</h3>
 ...
 background: selected.is_published ? "rgba(163,49,30,0.1)" : "rgba(31,71,56,0.1)",
 color: selected.is_published ? "#A3311E" : "#1F4738", fontSize: 12, fontWeight: 600, cursor: "pointer" }}>
 ...
 <div key={m.id} style={{ background: "rgba(28,38,32,0.02)", border: `1px solid ${S.border}`, borderRadius: 12, padding: "14px 16px" }}>
 <div style={{ display: "flex", alignItems: "center", gap: 12 }}>
 ...
 <span style={{ display: "inline-flex" }}>{(() => { const TI = typeIcon[m.type]; return TI ? <TI size={18} /> : null; })()}</span>
 <div style={{ flex: 1 }}>
 ...
 style={{ fontSize: 11, padding: "5px 10px", borderRadius: 7, background: "rgba(31,71,56,0.08)", border: `1px solid rgba(31,71,56,0.2)`, color: "#1F4738", cursor: "pointer" }}> Q&amp;A</button>
 <button onClick={() => openEditMaterial(m)}
 style={{ fontSize: 11, padding: "5px 10px", borderRadius: 7, background: "rgba(177,80,43,0.1)", border: `1px solid rgba(177,80,43,0.2)`, color: "#B1502B", cursor: "pointer" }}>Edit</button>
 <button onClick={() => deleteMaterial(m.id)}
 style={{ fontSize: 11, padding: "5px 10px", borderRadius: 7, background: "rgba(163,49,30,0.08)", border: `1px solid rgba(163,49,30,0.2)`, color: "#A3311E", cursor: "pointer" }}>Delete</button>
 </div>
 </div>
 {qaOpen === m.id && (
 ...
 <div style={{ background: "#F2EEE3", border: `1px solid ${S.border}`, borderRadius: 18, padding: 28, width: "100%", maxWidth: 480, display: "flex", flexDirection: "column", gap: 16 }}>
 <h3 style={{ fontSize: 16, fontWeight: 700, color: S.text, fontFamily: "inherit", margin: 0 }}>New Course</h3>
 <div>
 ...
 style={{ width: 36, height: 36, borderRadius: 9, fontSize: 18, cursor: "pointer", border: `1px solid ${cEmoji === e ? S.accent : S.border}`, background: cEmoji === e ? "rgba(177,80,43,0.12)" : "rgba(28,38,32,0.03)" }}>
 {e}
 </button>
 ))}
 </div>
 ...
 <button onClick={() => setShowCourseModal(false)} style={{ padding: "9px 18px", borderRadius: 9, background: "rgba(28,38,32,0.05)", border: `1px solid ${S.border}`, color: S.muted, fontSize: 13, cursor: "pointer" }}>Cancel</button>
 ...
 {showMaterialModal && (
 <div style={{ position: "fixed", inset: 0, background: "rgba(0,0,0,0.7)", zIndex: 100, display: "flex", alignItems: "center", justifyContent: "center", padding: 20 }}>
 <div style={{ background: "#F2EEE3", border: `1px solid ${S.border}`, borderRadius: 18, padding: 28, width: "100%", maxWidth: 540, display: "flex", flexDirection: "column", gap: 16, maxHeight: "90vh", overflowY: "auto" }}>
 <h3 style={{ fontSize: 16, fontWeight: 700, color: S.text, fontFamily: "inherit", margin: 0 }}>
 ...
 <option value="text" style={{ background: "#F2EEE3" }}> Text / Notes</option>
 <option value="video" style={{ background: "#F2EEE3" }}> Video Upload</option>
 <option value="embed" style={{ background: "#F2EEE3" }}> YouTube / Vimeo</option>
 <option value="pdf" style={{ background: "#F2EEE3" }}> PDF / Document</option>
 </select>
 </div>
 <div>
 ...
 <p style={{ fontSize: 11, color: "#1F4738", marginBottom: 6 }}>
 Current file: <a href={editMaterial.file_url} target="_blank" rel="noreferrer" style={{ color: "#1F4738" }}>view</a>
 </p>
 )}
 <input type="file"
 ...
 style={{ padding: "9px 18px", borderRadius: 9, background: "rgba(28,38,32,0.05)", border: `1px solid ${S.border}`, color: S.muted, fontSize: 13, cursor: "pointer" }}>
 Cancel
 </button>
