...
export function LinkPreviewCard({ url }: { url: string }) {
  ...
  return (
    <a href={data.url} target="_blank" rel="noreferrer" style={{ display: "block", marginTop: 6, borderRadius: 10, overflow: "hidden", border: "1px solid rgba(255,255,255,0.1)", textDecoration: "none", background: "rgba(255,255,255,0.03)", maxWidth: 260 }}>
      {data.image && <img src={data.image} alt="" style={{ width: "100%", maxHeight: 130, objectFit: "cover", display: "block" }} />}
      <div style={{ padding: "8px 10px" }}>
        <p style={{ fontSize: 12, fontWeight: 600, color: "#CDD6F4", margin: "0 0 2px", overflow: "hidden", textOverflow: "ellipsis", display: "-webkit-box", WebkitLineClamp: 2, WebkitBoxOrient: "vertical" }}>{data.title}</p>
        <p style={{ fontSize: 10, color: "#4A5170", margin: 0 }}>{new URL(data.url).hostname}</p>
      </div>
    </a>
  );
}
