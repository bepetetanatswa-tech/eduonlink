...
export function EcoCashPayment({ plan, creditPack, username, onSuccess, onBack }: Props) {
  ...
  return (
    <div style={{ display: "flex", flexDirection: "column", gap: 20, maxWidth: 520 }}>
      <button onClick={onBack} style={{ alignSelf: "flex-start", padding: "7px 14px", borderRadius: 9, background: "rgba(255,255,255,0.04)", border: `1px solid ${S.border}`, color: S.muted, fontSize: 12, cursor: "pointer" }}>
        ← Back
      </button>
      ...
      <div style={{ background: "rgba(0,229,163,0.04)", border: "1px solid rgba(0,229,163,0.15)", borderRadius: 16, padding: 20 }}>
        ...
        {[
          ...
          <div key={n} style={{ display: "flex", gap: 12, alignItems: "flex-start", marginBottom: 10 }}>
            <div style={{ width: 24, height: 24, borderRadius: "50%", background: "rgba(0,229,163,0.15)", border: "1px solid rgba(0,229,163,0.3)", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}>
              <span style={{ fontSize: 11, fontWeight: 700, color: "#00E5A3" }}>{n}</span>
            </div>
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
        ...
        <div>
          ...
          {file && <p style={{ fontSize: 11, color: "#00E5A3", marginTop: 4 }}>✓ {file.name} attached</p>}
        </div>
        {error && (
          <div style={{ background: "rgba(255,107,107,0.08)", border: "1px solid rgba(255,107,107,0.2)", borderRadius: 9, padding: "10px 14px" }}>
