...
import { IconCheck, IconBook } from "@/components/icons";
...
[{"question":"...","options":["A...","B...","C...","D..."],"answer":"A","explanation":"...","type":"mcq"}]`
 ...
 style={{ padding: "10px 20px", background: tab === t ? "rgba(177,80,43,0.15)" : "rgba(28,38,32,0.02)", border: "none", color: tab === t ? "#B1502B" : S.muted, fontSize: 13, fontWeight: tab === t ? 600 : 400, cursor: "pointer", textTransform: "capitalize" }}>
 {t === "papers" ? " Past Papers" : t === "practice" ? " AI Practice" : "⏱ Mock Exam"}
 </button>
 ))}
 </div>
 ...
 {SUBJECTS.map(s => <option key={s} value={s} style={{ background: "#F2EEE3" }}>{s}</option>)}
 </select>
 {isStaff && (
 ...
 {isStaff && showUpload && (
 <div style={{ background: "rgba(28,38,32,0.02)", border: `1px solid ${S.border}`, borderRadius: 14, padding: 18, display: "flex", flexDirection: "column", gap: 10 }}>
 ...
 {LEVELS.map(l => <option key={l} value={l} style={{ background: "#F2EEE3" }}>{l}</option>)}
 </select>
 ...
 <div key={p.id} style={{ background: "rgba(28,38,32,0.02)", border: `1px solid ${S.border}`, borderRadius: 14, padding: "16px" }}>
 ...
 {practiceLoading && (
 <div style={{ background: "rgba(177,80,43,0.05)", border: `1px solid rgba(177,80,43,0.15)`, borderRadius: 12, padding: "20px", textAlign: "center" }}>
 <p style={{ fontSize: 13, color: "#B1502B" }}>Sir Taks AI is generating your practice questions…</p>
 </div>
 )}
 ...
 return (
 <div key={i} style={{ background: "rgba(28,38,32,0.02)", border: `1px solid ${practiceChecked ? (correct ? "rgba(31,71,56,0.25)" : wrong ? "rgba(163,49,30,0.25)" : S.border) : S.border}`, borderRadius: 14, padding: "18px 20px" }}>
 <p style={{ fontSize: 14, fontWeight: 600, color: S.text, margin: "0 0 12px", lineHeight: 1.5 }}><span style={{ color: "#B1502B" }}>Q{i + 1}.</span> {q.question}</p>
 ...
 background: isCorrect ? "rgba(31,71,56,0.1)" : isWrong ? "rgba(163,49,30,0.1)" : isSelected ? "rgba(177,80,43,0.1)" : "rgba(28,38,32,0.03)",
 border: `1px solid ${isCorrect ? "rgba(31,71,56,0.3)" : isWrong ? "rgba(163,49,30,0.3)" : isSelected ? "rgba(177,80,43,0.3)" : S.border}`,
 color: isCorrect ? "#1F4738" : isWrong ? "#A3311E" : isSelected ? "#B1502B" : S.muted,
 fontSize: 13 }}>
 ...
 {practiceChecked && (
 <div style={{ marginTop: 12, padding: "10px 14px", background: "rgba(177,80,43,0.06)", border: `1px solid rgba(177,80,43,0.15)`, borderRadius: 9 }}>
 <p style={{ fontSize: 11, fontWeight: 600, color: "#B1502B", margin: "0 0 3px" }}>Explanation</p>
 <p style={{ fontSize: 12, color: S.muted, margin: 0, lineHeight: 1.5 }}>{q.explanation}</p>
 {q.type === "mcq" && <p style={{ fontSize: 12, color: "#1F4738", margin: "4px 0 0", fontWeight: 600 }}>Correct answer: {q.answer}</p>}
 </div>
 )}
 </div>
 );
 ...
 style={{ padding: "11px 24px", borderRadius: 10, background: S.accent, border: "none", color: "#fff", fontSize: 14, fontWeight: 700, cursor: "pointer", alignSelf: "flex-start", fontFamily: "inherit" }}>
 Check Answers
 </button>
 )}
 {practiceChecked && (
 <div style={{ padding: "14px 20px", borderRadius: 12, background: "rgba(31,71,56,0.08)", border: "1px solid rgba(31,71,56,0.2)" }}>
 <p style={{ fontSize: 14, fontWeight: 700, color: "#1F4738", margin: 0 }}>
 ...
 {!mockSession ? (
 <>
 <div style={{ background: "rgba(28,38,32,0.02)", border: `1px solid ${S.border}`, borderRadius: 14, padding: "24px" }}>
 <h3 style={{ fontSize: 15, fontWeight: 700, color: S.text, fontFamily: "inherit", margin: "0 0 16px" }}>Start Mock Exam</h3>
 ...
 {mockError && <p style={{ fontSize: 12, color: "#A3311E", margin: "10px 0 0" }}>{mockError}</p>}
 </div>

 {sessions.length > 0 && (
 <div>
 <h3 style={{ fontSize: 14, fontWeight: 600, color: S.text, fontFamily: "inherit", margin: "0 0 12px" }}>Past Results</h3>
 <div style={{ display: "flex", flexDirection: "column", gap: 8 }}>
 {sessions.map(s => {
 const pct = s.total > 0 ? Math.round((s.score / s.total) * 100) : 0;
 const color = pct >= 80 ? "#1F4738" : pct >= 60 ? "#A9873F" : "#A3311E";
 return (
 <div key={s.id} style={{ background: "rgba(28,38,32,0.02)", border: `1px solid ${S.border}`, borderRadius: 12, padding: "14px 16px", display: "flex", alignItems: "center", gap: 16 }}>
 <div style={{ flex: 1 }}>
 ...
 <p style={{ fontSize: 20, fontWeight: 700, color, fontFamily: "inherit", margin: 0 }}>{pct}%</p>
 <p style={{ fontSize: 10, color: S.dim, margin: 0 }}>{s.score}/{s.total}</p>
 </div>
 )}
 {!s.completed_at && <span style={{ fontSize: 11, color: "#A9873F" }}>Incomplete</span>}
 </div>
 );
 })}
 </div>
 ...
 <div style={{ display: "flex", alignItems: "center", gap: 16, padding: "12px 20px", background: mockSession.timeLeft < 300 ? "rgba(163,49,30,0.08)" : "rgba(177,80,43,0.06)", border: `1px solid ${mockSession.timeLeft < 300 ? "rgba(163,49,30,0.2)" : "rgba(177,80,43,0.15)"}`, borderRadius: 12 }}>
 ...
 <span style={{ fontSize: 22, fontWeight: 700, fontFamily: "monospace", color: mockSession.timeLeft < 300 ? "#A3311E" : "#1F4738" }}>
 {formatTime(mockSession.timeLeft)}
 </span>
 <button onClick={() => submitMock()}
 style={{ padding: "9px 16px", borderRadius: 9, background: "#1F4738", border: "none", color: "#000", fontSize: 12, fontWeight: 700, cursor: "pointer" }}>
 Submit
 </button>
 </div>
 )}
 ...
 <div style={{ padding: "24px", background: "rgba(31,71,56,0.06)", border: "1px solid rgba(31,71,56,0.2)", borderRadius: 14, textAlign: "center" }}>
 <div style={{ display: "flex", justifyContent: "center", marginBottom: 8, color: mockSession.score >= 8 ? "#1F4738" : mockSession.score >= 5 ? "#A9873F" : "#566257" }}>
                    {mockSession.score >= 8 ? <IconCheck size={36} strokeWidth={2} /> : <IconBook size={36} />}
                  </div>
 <p style={{ fontSize: 28, fontWeight: 700, color: "#1F4738", fontFamily: "inherit", margin: "0 0 4px" }}>
 ...
 return (
 <div key={i} style={{ background: "rgba(28,38,32,0.02)", border: `1px solid ${mockSession.submitted ? (isCorrect ? "rgba(31,71,56,0.25)" : isWrong ? "rgba(163,49,30,0.25)" : S.border) : S.border}`, borderRadius: 14, padding: "16px 18px" }}>
 <p style={{ fontSize: 14, fontWeight: 600, color: S.text, margin: "0 0 10px" }}><span style={{ color: "#B1502B" }}>Q{i + 1}.</span> {q.question}</p>
 ...
 background: showCorrect ? "rgba(31,71,56,0.1)" : showWrong ? "rgba(163,49,30,0.1)" : isSel ? "rgba(177,80,43,0.1)" : "rgba(28,38,32,0.02)",
 border: `1px solid ${showCorrect ? "rgba(31,71,56,0.3)" : showWrong ? "rgba(163,49,30,0.3)" : isSel ? "rgba(177,80,43,0.3)" : S.border}`,
 color: showCorrect ? "#1F4738" : showWrong ? "#A3311E" : isSel ? "#B1502B" : S.muted, fontSize: 13 }}>
 <span style={{ fontWeight: 700, marginRight: 8 }}>{letter}.</span>{opt}
 </button>
 );
 })}
